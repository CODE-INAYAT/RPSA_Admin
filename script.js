...
// Disable Developer Tools and other shortcuts for Win & Mac
document.addEventListener('keydown', function (e) {
    ...
    // Disable Ctrl+Shift+I, Ctrl+Shift+J, Ctrl+Shift+M, Ctrl+Shift+P for Windows
    if (e.ctrlKey && e.shiftKey && [73, 74, 77, 80].includes(e.keyCode)) {
        e.preventDefault();
    }

    // Disable Command+Option+I, Command+Option+J, Command+Shift+M, Command+Shift+P for Mac
    if (e.metaKey && (e.altKey || e.shiftKey) && [73, 74, 77, 80].includes(e.keyCode)) {
        e.preventDefault();
    }
    ...
    // Disable Command+Option+U (view source) for Mac
    if (e.metaKey && e.altKey && e.keyCode === 85) {
        e.preventDefault();
    }

    // Disable Ctrl+], Ctrl+[ (panel switching) for Windows
    if (e.ctrlKey && [221, 219].includes(e.keyCode)) {
        e.preventDefault();
    }

    // Disable Cmd+], Cmd+[ (panel switching) for Mac
    if (e.metaKey && [221, 219].includes(e.keyCode)) {
        e.preventDefault();
    }

    // Disable Ctrl+P (print) for Windows
    if (e.ctrlKey && e.keyCode === 80) {
        e.preventDefault();
        alert('Printing is disabled on this page.');
    }

    // Disable Cmd+P (print) for Mac
